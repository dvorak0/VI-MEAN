...
class EulerIntegration : public IntegrationBase<EulerIntegration>
{
  public:
    EulerIntegration(const Eigen::Vector3d &_acc_0, const Eigen::Vector3d &_gyr_0,
                     const Eigen::Vector3d &_linearized_ba, const Eigen::Vector3d &_linearized_bg)
        : IntegrationBase<EulerIntegration>{_acc_0, _gyr_0, _linearized_ba, _linearized_bg}
    {
    }
        ...
        Eigen::MatrixBase<Derived1> &delta_v, Eigen::MatrixBase<Derived1> &linearized_ba, Eigen::MatrixBase<Derived1> &linearized_bg)
    {
        ...
        Eigen::Matrix<Scalar_t, 3, 1> un_acc = tmp_delta_q * (tmp_acc_0.template cast<Scalar_t>() - tmp_linearized_ba);
        Eigen::Matrix<Scalar_t, 3, 1> un_gyr = tmp_gyr_0.template cast<Scalar_t>() - tmp_linearized_bg;

        delta_p = tmp_delta_p + tmp_delta_v * dt_t + static_cast<Scalar_t>(0.5) * dt_t * dt_t * un_acc;
        delta_q = tmp_delta_q * Utility::deltaQ(un_gyr * dt_t);
        delta_v = tmp_delta_v + un_acc * dt_t;
